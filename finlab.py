...
def get_benchmark(freq="15T"):
    res = requests.get('https://drive.google.com/uc?export=download&id=1iFVLfFS7MgDF2BOJAkFNgDHapD9dFQzb')
  
    f = open('benchmark.pkl.gz', 'wb')
    f.write(res.content)
    f.close()
    return pd.read_pickle('benchmark.pkl.gz', compression='gzip').resample(freq).first().dropna()
...
def create_label_trade_point_dicision(b, th=200):
    th = abs(th)
...
def create_label_continuous_trading_signal(s, n):
    ...
    ret = s.rolling(n).apply(generate_signal, raw=True).shift(-n+1)
    ret.name = 'continuous_trading_signal'
    return ret


def create_label_triple_barrier(price, ub, lb, max_period):
...
def create_label_fixed_time_horizon(s, time):    
    
    ret = pd.Series(0, index=s.index)

    returns = s.shift(-time) / s
    ub = returns.quantile(2/3)
    lb = returns.quantile(1/3)
    ret[returns > ub] = 1
    ret[returns < lb] = -1
    
    ret.name = 'fixed_time_horizon_' + str(time)
